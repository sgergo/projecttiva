...
static void board_spi_cspin_init(void) {
	ROM_SysCtlPeripheralEnable(SPIPINCSPERIPHERIAL);
	ROM_GPIOPinTypeGPIOOutput(SPIPINCSPERIPHERIALBASE, SPICSPIN);
	ROM_GPIOPinWrite(SPIPINCSPERIPHERIALBASE, SPICSPIN, SPICSPIN); // Pull CS high
}

void board_spi_cspin_high(void) {
	ROM_GPIOPinWrite(SPIPINCSPERIPHERIALBASE, SPICSPIN, SPICSPIN);
}

void board_spi_cspin_low(void) {
	ROM_GPIOPinWrite(SPIPINCSPERIPHERIALBASE, SPICSPIN, 0);
}
#endif
